...
public class ParticleEmitterBox2DScalable extends ParticleEmitter {
	
	final World world;
	final Vector2 startPoint = new Vector2();
	final Vector2 endPoint = new Vector2();
	Vector2 startPointScaled = new Vector2();
	Vector2 endPointScaled = new Vector2();
	/** collision flag */
	...
	final RayCastCallback rayCallBack = new RayCastCallback() {
		public float reportRayFixture (Fixture fixture, Vector2 point, Vector2 normal, float fraction) {
			ParticleEmitterBox2DScalable.this.particleCollided = true;
			ParticleEmitterBox2DScalable.this.normalAngle = MathUtils.atan2(normal.y, normal.x) * MathUtils.radiansToDegrees;
			return fraction;
		}
	};
	 ...
	 * 
	 * @param world */
	public ParticleEmitterBox2DScalable (World world) {
		super();
		this.world = world;
	}
	 ...
	 * @param reader
	 * @throws IOException */
	public ParticleEmitterBox2DScalable (World world, BufferedReader reader) throws IOException {
		super(reader);
		this.world = world;
	}
	 ...
	 * @param world
	 * @param emitter */
	public ParticleEmitterBox2DScalable (World world, ParticleEmitter emitter) {
		super(emitter);
		this.world = world;
	}
...
//			final float y = getY();
			...
			startPointScaled.set(x*C.WORLD_TO_BOX, y*C.WORLD_TO_BOX);
			endPointScaled.set((x+velocityX)*C.WORLD_TO_BOX, (y+velocityY)*C.WORLD_TO_BOX);
			if (world != null) world.rayCast(rayCallBack, startPointScaled, endPointScaled);

			/** If ray collided boolean has set to true at rayCallBack */
			if (particleCollided) {
				// perfect reflection
				angle = 2*normalAngle - angle - 180f;
				angle = angle + Rnd.randomInt(-10, 10);
