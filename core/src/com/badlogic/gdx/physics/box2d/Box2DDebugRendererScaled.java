...
public class Box2DDebugRendererScaled {
	...
	public Box2DDebugRendererScaled () {
		this(true, true, false, true, false);
	}

	public Box2DDebugRendererScaled (boolean drawBodies, boolean drawJoints, boolean drawAABBs, boolean drawInactiveBodies,
	...
	public void render (World world, Matrix4 projMatrix) {
		...
		try{
			renderBodies(world);
		} catch(Exception e) {renderer.end();}
	}
	...
	private void renderBodies (World world) {
		...
		if (drawBodies || drawAABBs) {
			for(Body b : bodyes)
				if (b.isActive() || drawInactiveBodies) renderBody(b);
		}
	...
	private void drawShape (Fixture fixture, Transform transform, Color color) {
		if (fixture.getType() == Type.Circle) {
			...
			transform.mul(t);
			t.scl(C.BOX_TO_WORLD);
			drawSolidCircle(t, circle.getRadius()*C.BOX_TO_WORLD, axis.set(transform.vals[Transform.COS], transform.vals[Transform.SIN]), color);
		}
		...
		if (fixture.getType() == Type.Edge) {
			...
			vertices[0].scl(C.BOX_TO_WORLD);
			vertices[1].scl(C.BOX_TO_WORLD);
	...
	public static void setAxis (Vector2 axis) {
		Box2DDebugRendererScaled.axis = axis;
	}
