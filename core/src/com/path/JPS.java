...
public class JPS {
	...
	public List<Node> search(){
		...
		System.out.println("Start X: "+startX+" Y: "+startY +" free? "+ grid.getNode(startX, startY).pass);  //Start and End points are printed for reference
		System.out.println("End   X: "+endX+" Y: "+endY +" free? "+ grid.getNode(endX, endY).pass);
...
//				if (draw){grid.drawStart(startX, startY); grid.drawEnd(endX, endY); grid.picPrint("2 - JumpPoints");} //draw start, end, and print the picture sans path
				path = grid.pathCreate(cur);    //the path is then created
